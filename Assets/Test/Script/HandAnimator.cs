...
public sealed class HandAnimator : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        // }
        var lowerJoint = _pipeline.GetKeyPoint(13);
        var upperJoint = _pipeline.GetKeyPoint(14);
        var farRight = _pipeline.GetKeyPoint(2);
        var farLeft = _pipeline.GetKeyPoint(17);
        var direction = upperJoint - lowerJoint;
        sphere.transform.position = lowerJoint + direction * 0.5f;

        sphere.transform.localScale = Vector3.one * Vector3.Distance(farLeft, farRight) / 5f;

        // var angle = Vector3.Angle(Vector3.up, direction);
        // sphere.transform.Rotate(0.0f, 0.0f, angle - gameObject.transform.eulerAngles.z - 180f);
        sphere.transform.rotation = Quaternion.LookRotation(new Vector3(direction.x, direction.y, 0));
        sphere.transform.Rotate(0.0f, 90f, 0.0f);
